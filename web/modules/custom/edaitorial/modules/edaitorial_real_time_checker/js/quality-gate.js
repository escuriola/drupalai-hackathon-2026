  ...
  Drupal.behaviors.edaitorialQualityGate = {
    attach: function (context, settings) {
        ...
        if ($statusCheckbox.length) {
          ...
          if (!$('#quality-gate-status-notice').length && $statusWrapper.length) {
            $statusWrapper.append(
              ...
              '✓ <strong>Quality score: ' + score + '%</strong> (required: ' + minScore + '%). Ready to publish.' +
              '</div>'
            );
          }
        }
      ...
      function analyzeContent() {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (canPublish) {
              $('.analyze-status').html('✓ Analysis complete — Score: ' + response.score + '%. Ready to publish.');
              Drupal.announce(
                Drupal.t('Quality score: @score%. Ready to publish.', {'@score': response.score}),
                'polite'
              );
            } else {
              $('.analyze-status').html('Score: ' + response.score + '% — Minimum ' + minScore + '% required for publication.');
              Drupal.announce(
      ...
      function updateSidebarScores(response) {
        ...
        const statusIcon = canPublish && score > 0 ? '✓' : (score > 0 ? '✗' : '⏸');
      ...
      function displayResultsTab(response) {
        ...
        html += '<div class="badge-icon">' + (canPublish ? '✓' : '✗') + '</div>';
        ...
        if (!canPublish && !blockPublishing) {
          ...
          html += '<div class="recommendation-icon">ℹ️</div>';
        ...
        if (response.issues && response.issues.length > 0) {
          ...
          const categoryConfig = {
            'SEO': { icon: '🔍', color: 'seo' },
            'Typos': { icon: '✍️', color: 'typos' },
            'Links': { icon: '🔗', color: 'links' },
            'Content': { icon: '📝', color: 'content' },
            'Accessibility': { icon: '♿', color: 'accessibility' }
          };
          ...
          for (const [category, issues] of Object.entries(issuesByCategory)) {
            if (issues.length > 0) {
              const config = categoryConfig[category] || { icon: '•', color: 'other' };
              ...
              sortedIssues.forEach(function(issue, index) {
                ...
                html += '<span class="expand-icon">ℹ️</span>';
                html += '</button>';
                html += '</div>';
          ...
          if (!$('#quality-gate-warning').length) {
            let $warning;
              ...
              $warning = $('<div id="quality-gate-warning" class="messages messages--error quality-gate-blocked">')
                .html(
                  '<strong>Publication blocked</strong> — Quality score: <strong>' + score + '%</strong> (required: <strong>' + minScore + '%</strong>). ' +
      ...
      function showInlineLoadingState(field) {
        ...
        const html = '<div class="ai-result-header">' +
          ...
          '<p>✨ Evaluating the <strong>' + fieldName + '</strong> field content</p>' +
      ...
      function showInlineSuggestion(field, data) {
        ...
        html += '<div class="ai-result-title"><span style="font-size: 20px;">✨</span> AI Assistant</div>';
      ...
      function showAiSuggestionModal(data) {
        ...
        html += '<h2><span style="font-size: 24px;">✨</span> AI Assistant</h2>';
